...
{
    [SerializeField]
    private GameObject startPlatform, endPlatform, platfromPrefabs;

    private float blockWidhth = 0.5f, blockHeight = 0.2f;
    ...
    void InstantiateLevel()
    {
        for(int i = beginAmount; i < amountToSpawn; i++)
        {
            ...
            {
                newPlatform = Instantiate(platfromPrefabs);
            }
            ...
            if(left == 0)
            {
                newPlatform.transform.position = new Vector3(lastPos.x - blockWidhth, lastPos.y + blockHeight, lastPos.z);
            }
            else
            {
                newPlatform.transform.position = new Vector3(lastPos.x , lastPos.y + blockHeight, lastPos.z + blockWidhth);
            }
